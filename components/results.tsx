...
export default function Results({ runId, demo, api, onBack, onRetry }: { runId: string; demo: boolean; api: (path: string, options?: RequestInit) => Promise<any>; onBack: () => void; onRetry: (listingId: string, rows: ResultRow[], officialTemplateId?: string) => void }) {
  ...
  useEffect(() => {
    let active = true; let timeout: ReturnType<typeof setTimeout>; let resultsComplete = false; let observedCompletion = false;
    async function update() {
      try {
        const [d, r] = await Promise.all([api(`/runs/${runId}`), resultsComplete ? Promise.resolve(null) : api(`/runs/${runId}/results`)]);
        if (!active) return;
        setDetail(d); setError('');
        if (r) { setRows(r.items || []); setPageToken(r.nextPageToken || ''); }
        // Fetch once more after observing completion so a racing results response
        // cannot leave the last task unfinished. Settlement-only polls retain loaded pages.
        resultsComplete = observedCompletion && terminal(d.run.status);
        observedCompletion = terminal(d.run.status);
        if (shouldPollRun(d.run, d.market || d.run.market) || !resultsComplete) timeout = setTimeout(update, 8000);
  ...
  const summaryHint = run && terminal(run.status)
    ? marketSettlementPending(run, market)
      ? '任务已结束，费用待结算，正在自动更新。当前预估金额不是最终扣费。'
      : '结果已更新。产物链接过期时，请点击刷新重新获取。'
    : '任务在云端继续运行，关闭本页后也可从我的任务查看。';
