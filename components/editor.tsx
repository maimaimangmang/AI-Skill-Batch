...
import { base64, download, exportSheet, post } from '@/lib/client';
...
type Props = { listing: Listing; schema: Schema; rows: InputRow[]; setRows: (rows: InputRow[]) => void; demo: boolean; busy: boolean; errors: string[]; api: (path: string, options?: RequestInit) => Promise<any>; onBack: () => void; onQuote: () => void };
export default function Editor({ listing, schema, rows, setRows, demo, busy, errors, api, onBack, onQuote }: Props) {
  ...
  return <>
    ...
    {uploading && <p className="notice"><LoaderCircle className="spin" size={16} />正在处理文件…</p>}
