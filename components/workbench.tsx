...
import type { Balance, InputRow, Listing, Quote, ResultRow, Run, Schema } from '@/lib/types';
import { cellText, emptyRow, money, normalizeRows, schemaOf, statusLabel } from '@/lib/domain';
import { post, request, RequestError } from '@/lib/client';
import { demoRequest } from '@/lib/demo';
import { declaredModels, MARKET_PAGE_SIZE, marketPageNumbers, collectMarket } from '@/lib/market';
...
export default function Workbench() {
  ...
  const [balance, setBalance] = useState<Balance | null>(null); const [marketListings, setListings] = useState<Listing[]>([]); const [runs, setRuns] = useState<Run[]>([]);
  ...
  const marketPageCount = Math.max(1, Math.ceil(marketListings.length / MARKET_PAGE_SIZE));
  const listings = marketListings.slice(marketPage * MARKET_PAGE_SIZE, (marketPage + 1) * MARKET_PAGE_SIZE);
  const scrollMarketAfterLoad = useRef(false);
  const [runToken, setRunToken] = useState(''); const [refresh, setRefresh] = useState(0);
  ...
  const api = useCallback(async (path: string, options?: RequestInit) => {
    ...
    try { return await request(path, options); } catch (e) {
      if (e instanceof RequestError && e.status === 401) { setConnected(false); setKey(''); setListings([]); setMarketPage(0); setConfirmation(null); setRows([]); setListing(null); setRuns([]); setBalance(null); setPending([]); setLoginError(e.message); }
      throw e;
    }
  }, [demo]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!connected || !['market', 'runs'].includes(view)) return;
    let active = true; const controller = new AbortController(); setLoading(true); setError('');
    if (view === 'market') setListings([]);
    const result = view === 'market'
      ? collectMarket(api, query, controller.signal)
      : api('/runs', { signal: controller.signal });
    result.then(data => {
      if (!active) return;
      if (view === 'market') {
        setListings(data.items || []);
        setMarketPage(page => Math.min(page, Math.max(0, Math.ceil(data.items.length / MARKET_PAGE_SIZE) - 1)));
      } else { setRuns(data.items || []); setRunToken(data.nextPageToken || ''); }
    }).catch(e => { if (active) setError(e.message); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; controller.abort(); };
  }, [api, connected, query, view, refresh]);
  useEffect(() => {
  ...
  async function logout() {
    ...
    setConnected(false); setDemo(false); setKey(''); setListings([]); setMarketPage(0); setRuns([]); setRows([]); setListing(null); setBalance(null); setConfirmation(null); setPending([]); setView('market'); setError(''); setLoginError(''); window.history.replaceState({}, '', '/');
  }
  ...
  return <div className={`app-shell studio-app${view === 'market' ? ' studio-market' : ' studio-workspace'}`}>
    ...
    <main className="main-content">
      ...
      {view === 'market' && <>
        ...
        <nav className="market-pagination" aria-label="工作流市场分页">
          ...
          <div className="pagination-buttons"><button className="button small" disabled={loading || marketPage === 0} onClick={() => goToMarketPage(marketPage - 1)}>上一页</button>
            ...
            <button className="button small" disabled={loading || marketPage >= marketPageCount - 1} onClick={() => goToMarketPage(marketPage + 1)}>下一页</button></div>
        </nav>
      ...
      {view === 'runs' && <><div className="page-heading"><div><div className="eyebrow">MY RUNS</div><h1>我的任务<span className="heading-dot">.</span></h1><p>查看运行进度，把每一项结果带回你的工作。</p></div><button className="button" onClick={() => setRefresh(r => r + 1)}><RefreshCw size={16} />刷新任务</button></div><section className="editor-card"><div className="table-toolbar"><div><ListChecks size={20} /><strong>运行记录</strong></div><button className="button small primary" onClick={() => navigate('market')}><Play size={14} />新建任务</button></div>{loading && !runs.length ? <div className="loading"><LoaderCircle className="spin" />正在获取任务…</div> : runs.length ? <div className="table-scroll"><table className="runs-table"><thead><tr><th>工作流 / 任务</th><th>提交时间</th><th>状态</th><th>任务进度</th><th /></tr></thead><tbody>{runs.map(run => <tr key={run.runId}><td><strong>{runName(run)}</strong><small className="mono">{run.runId}</small></td><td>{run.createdAtUnix ? new Date(run.createdAtUnix * 1000).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : '—'}</td><td><span className={`status ${run.status}`}>{statusLabel(run.status)}</span></td><td>{run.completedTasks ?? 0} / {run.totalTasks ?? '—'} 成功</td><td><button className="text-button" onClick={() => { setRunId(run.runId); navigate('detail'); }}>查看结果<ArrowRight size={15} /></button></td></tr>)}</tbody></table></div> : <div className="empty"><ListChecks size={36} /><h2>第一份结果，从这里开始</h2><p>到市场选择一个工作流，填写任务表后即可运行。</p><button className="button primary" onClick={() => navigate('market')}>浏览工作流<ArrowRight size={15} /></button></div>}{runToken && <button className="button load-more" disabled={loading} onClick={loadMore}>加载更多任务</button>}</section></>}
