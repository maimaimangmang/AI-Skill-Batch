...
import { RUNS_PAGE_SIZE, RUN_STATUS_OPTIONS, localDateBounds, localDateString as dateString, relativeRunQuery, type RunsPage } from '@/lib/runs';
...
export default function RunsView({ active, api, refreshKey, onNew, onResult }: Props) {
  ...
  const [currentDay, setCurrentDay] = useState(() => dateString(new Date()));
  useEffect(() => {
    const update = () => setCurrentDay(dateString(new Date()));
    update();
  ...
  const effectiveQuery = useMemo(() => relativeRunQuery(filterQuery, range, currentDay), [filterQuery, range, currentDay]);
  ...
  const pager = useMemo(() => {
    const fetchPage = (token: string, signal: AbortSignal): Promise<RunsPage> => {
      const params = new URLSearchParams(effectiveQuery); if (token) params.set('pageToken', token);
      return api(`/runs?${params}`, { signal });
    };
    return directPaging ? createOffsetPager(fetchPage, RUNS_PAGE_SIZE) : createCursorPager(fetchPage);
  }, [api, effectiveQuery, revision, refreshKey, directPaging]);
  ...
  return <>
    ...
    <section className="editor-card runs-panel" id="runs-panel">
      ...
      <form className="runs-filters" onSubmit={e => { e.preventDefault(); apply(); }}>
        ...
        <select aria-label="提交时间范围" value={range} onChange={e => { setRange(e.target.value); apply(status, e.target.value); }}><option value="all">全部时间</option><option value="1">今天（{currentDay}）</option><option value="7">近 7 天</option><option value="30">近 30 天</option><option value="custom">自定义日期</option></select>
      ...
      {loading ? <div className="loading" role="status"><LoaderCircle className="spin" />正在获取任务…</div> : runs.length ? <div className="table-scroll"><table className="runs-table"><thead><tr><th>工作流 / 任务</th><th>提交时间</th><th>状态</th><th>任务进度</th><th className="run-cost">费用</th><th /></tr></thead><tbody>{runs.map(run => { const cost = runCost(run); return <tr key={run.runId}><td><strong>{runName(run)}</strong><small className="mono">{run.runId}</small></td><td>{run.createdAtUnix ? new Date(run.createdAtUnix * 1000).toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : '—'}</td><td><span className={`status ${run.status}`}>{statusLabel(run.status)}</span></td><td>{run.completedTasks ?? 0} / {run.totalTasks ?? '—'} 成功</td><td className="run-cost"><strong>{cost.amount}</strong>{cost.label && <small>{cost.label}</small>}</td><td><button className="text-button" onClick={() => onResult(run.runId)}>查看结果<ArrowRight size={15} /></button></td></tr>; })}</tbody></table></div> : !error && <div className="empty"><ListChecks size={32} /><h2>{current?.nextPageToken ? '这批记录中暂无匹配任务' : filtered ? '没有找到匹配的任务' : '还没有运行任务'}</h2><p>{current?.nextPageToken ? '点击下一页，继续查找更早的任务。' : filtered ? '试试更换关键词、放宽时间范围或重置筛选。' : '到市场选择工作流，填写任务表后即可运行。'}</p>{filtered ? <button className="button" onClick={reset}>重置筛选</button> : <button className="button primary" onClick={onNew}>浏览工作流<ArrowRight size={15} /></button>}</div>}
      ...
      <nav className="market-pagination runs-pagination" aria-label="我的任务分页">
        <span className="pagination-summary" aria-live="polite">{loading ? '正在读取任务…' : totalCount != null ? `第 ${page + 1} / ${pageCount} 页 · 共 ${totalCount} 项` : `第 ${page + 1} 页 · 本页 ${runs.length} 项${current?.nextPageToken ? " · 总页数待确认" : ""}`} · 每页 {RUNS_PAGE_SIZE} 项</span>
