...
- 我的任务：每页 12 条按需分页，支持首页、尾页、名称或编号搜索、状态及日期筛选。无名称或日期条件时可直接请求尾页；名称与日期搜索按批查找历史记录，尚未确定总页数时禁用尾页入口，保留下一页继续查找。已浏览页面使用内存缓存。
- 首次只请求 12 项，点击下一页再请求下一批；已浏览页面缓存在内存中，切换搜索或刷新时重新从第一页加载。上游未返回总数，因此不预先读取全部列表计算总页数。
...
- 下载接口提供的输入模板；图片上传支持预览、替换、删除，也可填写 HTTPS 直链。附件提交使用接口返回的资产 ID。
- 先校验，再预估费用，用户明确确认后执行。
